...
class SocialStatusScreen extends Screen
{
   /**
     * Query data.
     *
    ...
    public function layout(): array
    {
        return [
            Layout::table('ssData', [
                ...
                TD::make('status_id', 'Статус')->render(function (SocialStatus $ss) {
                    return $ss->status_id == 1 ? "Активный" : "Неактивный";
                }),
                TD::make('action', "Действия")->render(function (SocialStatus $ss) {
                    return Group::make([
    ...
    public function asyncGetSS(SocialStatus $ss): array
    {
        return [
            'ssSinglData' => SocialStatus::find($ss->id)
        ];
    }
    ...
    {
        $ss = SocialStatus::find($request->ssSinglData['id']);
