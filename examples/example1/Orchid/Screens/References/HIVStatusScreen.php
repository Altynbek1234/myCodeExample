...
use \App\Models\HIVStatus;

class HIVStatusScreen extends Screen
{
    /**
     * Query data.
     *
    ...
    public function layout(): array
    {
        return [
            Layout::table('hsData', [
                ...
                TD::make('status_id', 'Статус')->render(function (HIVStatus $hs) {
                    return $hs->status_id == 1 ? "Активный" : "Неактивный";
                }),
                TD::make('action', "Действия")->render(function (HIVStatus $hs) {
                    return Group::make([
    ...
    public function asyncGetOLF(HIVStatus $hs): array
    {
        return [
            'hsSinglData' => HIVStatus::find($hs->id)
        ];
    }
    ...
    {
        $hs = HIVStatus::find($request->hsSinglData['id']);
